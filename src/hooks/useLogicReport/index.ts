...
const useLogicReport = () => {
  ...
  const formatReport = (
    ...
    reportsList: IReportData[]
  ) => {
    let filteredDataObj: any = getFilteredData(reportsList);

    let finalProjectData = Object.keys(filteredDataObj).map(element => {
      let totalAmount = 0;
      let projectDetails: IProjectData | undefined = projectsList.find(
        (project: IProjectData) => project.projectId === element
      );
      filteredDataObj[element].map(
        (transaction: ITransaction) => (totalAmount += transaction.amount)
      );
      return {
        ...projectDetails,
        transactions: filteredDataObj[element],
  ...
  const formatChartData = (
    ...
    let finalChartData = Object.keys(filteredDataObj).map(element => {
      let totalAmount = 0;
      let projectDetails: IProjectData | undefined = projectsList.find(
        project => project.projectId === element
      );
      filteredDataObj[element].map(
        (transaction: ITransaction) => (totalAmount += transaction.amount)
      );
      return {
        label: !!projectDetails && projectDetails.name,
